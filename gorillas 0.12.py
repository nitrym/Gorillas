...
# Fenster und Canvas
window = tk.Tk()
window.title("Gorillas mit Gebäudeschaden")
canvas = tk.Canvas(window, width=800, height=600, bg="skyblue")
canvas.pack()

# Gebäude speichern als Rechtecke mit Position
buildings = []
building_width = 50

# Zerstörung pro Gebäude: Liste mit Trefferpunkten (y-Positionen)
building_hits = {}
...
for i in range(0, 800, building_width):
    ...
    building_hits[i] = []  # Trefferliste initialisieren
    ...
    def animate():
        ...
        for building_id, x1b, y1b, x2b, y2b in buildings:
            if x1b < bx1 < x2b and y1b < by1 < y2b:
                ...
                # Neue Einschlagposition etwas nach unten bei Mehrfachtreffern
                base_x = int(x1b)
                hits = building_hits[base_x]
                y_hit = int(by1)

                # Prüfen, ob bereits getroffen wurde in diesem Bereich
                for past_y in hits:
                    if abs(past_y - y_hit) < 20:
                        y_hit += 20  # tieferes Loch

                hits.append(y_hit)

                # Explosion zeichnen
                explosion = canvas.create_oval(bx1 - 10, y_hit - 10, bx1 + 10, y_hit + 10, fill="black", outline="black")
                canvas.tag_raise(explosion)
                return
